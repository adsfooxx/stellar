...
using ApplicationCore.LinePay.Dtos.Capture;
using ApplicationCore.LinePay.Dtos.Confirm;
using ApplicationCore.LinePay.Dtos.Request;
using ApplicationCore.LinePay.Extensions;
using ApplicationCore.LinePay.Helpers;
using ApplicationCore.LinePay.Void;
using ApplicationCore.LinePayApiSdk.Dtos.CheckPaymentStatus;
using ApplicationCore.LinePayApiSdk.Dtos.CheckRegKey;
using ApplicationCore.LinePayApiSdk.Dtos.ExpireRegKeyResponse;
using ApplicationCore.LinePayApiSdk.Dtos.PaymentDetail;
using ApplicationCore.LinePayApiSdk.Dtos.PayPreapproved;
using ApplicationCore.LinePayApiSdk.Dtos.Refund;
using Microsoft.AspNetCore.Http;
using Microsoft.Extensions.Configuration;
using System.Text;
namespace ApplicationCore.LinePay
{
    public class LinePayApi
    {
        ...
        public string _sandboxApiBaseAddress, _productionApiBaseAddress;
        public LinePayApi(LinePayApiOptions options, IConfiguration configuration)
        {

            _sandboxApiBaseAddress = configuration.GetSection("LinePayApi:SandboxApiBaseAddress").Value;
            _productionApiBaseAddress = configuration.GetSection("LinePayApi:ProductionApiBaseAddress").Value;
            ...
            if (string.IsNullOrEmpty(options.BaseAddress))
            {
                _httpClient.BaseAddress = options.IsSandBox
                    ? new Uri(_sandboxApiBaseAddress)
                    : new Uri(_productionApiBaseAddress);
            }
            else
            {
        ...
        private Dictionary<string, string> GetRequestHeadersHandler(string requestContentString, string requestUriPath)
        {
            ...
            var signature =
               Helpers.SignatureHelper.CalculateSignature(_channelSecret, requestUriPath, requestContentString, nonce);
